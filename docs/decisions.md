...
- **Decision:**
  ...
  2. The confirmation is a **second post**, not a browser dialog. The first shows the titles about
     ...
     Afterwards the list comes back with the same filters, order and grouping, and says how many went.
...
- **Alternatives:** a `confirm()` dialog — it needs JavaScript, and "OK" on a dialog is easy to
  ...
  selecting beyond the page — "all" means the cards on screen, which "Show more" can raise to
  600, never entries the person has not seen.
- **Revisit when:** someone needs to select more than one page can show; a second
...
- **Status:** accepted as a hypothesis (2026-09-23), pending a measured batch. Complements the
  first-prompt lookup hook, proposed separately; builds on [0069](#adr-0069).
...
- **Alternatives:**
  ...
  - The first-prompt lookup hook is a complement, not an alternative: the hook asks for the
    lookup, this removes the step that made the lookup cost two decisions. Where the tools are
    already loaded, the hook's `ToolSearch` step is harmless.
- **Revisit when:** the next batch — `alwaysLoad` plus the first-prompt hook, with the memory frozen —
