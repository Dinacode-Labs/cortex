...
| 6 | P1 | Seguridad | ✅ **HECHO** (2026-09). `scrub()` vive en `@cortex/shared` y se aplica en tres puntos: antes del LLM en `captureCondensedViaApi` (cubre codex/opencode/hermes/reunión, no solo Claude), y en `saveContext`/`captureBatch` al persistir (el servidor no confía en el cliente). Patrones nuevos: connection strings, Cookie/Set-Cookie, Basic, sk-ant/npm_/Stripe. Tests: `tests/scrub.test.ts` + integración. | high | S |
| 7 | P1 | Modelo de datos | ✅ **HECHO** (2026-09). `resolveEntities` agrupa por `${type}:norm(name)`; homónimos de tipos distintos ya no se fusionan. Además el desempate de la canónica incluye el `id`, así que la fusión es determinista (esto causaba un test de integración flaky). Test de homónimos añadido. | high | S |
...
| 13 | P2 | Arquitectura | `relate()` es check-then-act sin constraint único → relaciones duplicadas bajo concurrencia. Índice único + `ON CONFLICT DO NOTHING`, eliminar el SELECT previo. | ✅ HECHO (mig 0013) · medium | S |
...
| 16 | P2 | Modelo de datos | `relations` sin unicidad → `relate()` TOCTOU duplica aristas (incl. contradicts/belongs_to). UNIQUE `(source_id, target_id, relation_type)` + `ON CONFLICT DO NOTHING`. | ✅ HECHO (mig 0013) · medium | S |
...
| 21 | P2 | Cobertura del plan | ✅ **HECHO** (2026-09, con #6). `scrub()` centralizado en `@cortex/shared`, aplicado en `captureCondensedViaApi` antes de cualquier LLM y al persistir; cubre connection strings y cookies. | medium | S |
