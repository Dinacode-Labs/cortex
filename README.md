...
Cortex ships **its own** pieces, the MCP with the 8 tools, the `cortex-recall` and
`cortex-capture` skills and the `/cortex-save` command, in the
...
(`Showing 38 of 349 entries`) and every section that had to cut ends in the argument that reads the
...
learned the hard way ([ADR-0069](./docs/decisions.md#adr-0069)). And because Claude Code loads MCP tools on
demand, the first prompt of each session gets its first action written out — load Cortex's read
tools and search with the user's words, before any command or file read
