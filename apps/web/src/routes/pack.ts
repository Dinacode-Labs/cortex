...
packRoutes.get("/pack", async (c) => {
  ...
  try {
    ...
    return c.html(layout("Context pack", html`<p><a class="back" href="/">← Inicio</a></p><div class="empty">Proyecto no encontrado.</div>`), 404);
  }
  ...
  const body = html`
    ...
    <form class="row" method="get" action="/pack" style="margin-bottom:16px">
      ...
      <input type="text" name="area" placeholder="Área/módulo, p.ej. facturación" value="${area ?? ""}">
      <input type="date" name="asof" value="${asOfStr ?? ""}" title="Point-in-time: contexto vigente a esta fecha">
      <button type="submit">Generar</button>
    </form>
    ${asOf ? html`<div class="warn">⏳ Vista <b>point-in-time</b>: hechos vigentes el ${asOfStr ?? ""} (incluye los que después se invalidaron).</div>` : ""}
    ...
    ${pack.sensitiveModules.length ? html`<div class="panel"><h2>Módulos sensibles</h2>${joinHtml(pack.sensitiveModules.map((m) => badge(m, "#bc4c00")), " ")}</div>` : ""}
    ${pack.relevantToArea.length ? html`<div class="panel"><h2>Relevante para "${area ?? ""}"</h2>${pack.relevantToArea.map((h) => html`<div style="margin-bottom:8px">${badge(h.score.toFixed(2), "#0099ff")} ${h.entry.title}</div>`)}</div>` : ""}
  `;
